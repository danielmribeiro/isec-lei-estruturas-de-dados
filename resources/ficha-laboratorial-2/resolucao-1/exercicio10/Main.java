package exercicio10;

import java.util.Arrays;
import java.util.Random;

public class Main {
    static int[] criaArrayCom(int valor, int dimensao, boolean diferentes){
        int m[]=new int[dimensao];
        if(diferentes){
            for (int i = 0; i < dimensao; i++) {
                m[i]=i*10;
            }
            if((valor%10!=0)||(0>valor)||(valor>(dimensao-1)*10))
                m[0]=valor;
        } else{
            Random r = new Random();
            int gama=(Math.abs(valor)<10)?10:Math.abs(valor);
            for (int i = 0; i < dimensao; i++) {
                m[i]=r.nextInt(gama*4)-gama*2;
            }
            m[0]=valor;
        }
        Arrays.sort(m);
        return m;
    }

    public static void main(String[] args) {
        //int array[]=criaArrayCom(32,11,true);
        //int array[]={3,7,12,15};
        int array[]={-3,1,7,12,15};
        //int array[]={-15,-14,1,2,3,4};
        for (int i = 0; i < array.length; i++) {
            System.out.print(array[i]+",");
        }
        System.out.println();
        int resultado = pesquisaBinaria(array);
        System.out.println(resultado);
    }

    private static int pesquisaBinaria(int[] tabela) {
        int start=0;
        int end=tabela.length-1;
        int meio=(start+end)/2;
        do {
            if (tabela[meio] > meio) {
                end = meio;
            }else{
                start = meio + 1;
            }
            meio = (start + end) / 2;
        }while(start<end);
        if(tabela[meio] > meio) {
            return meio;
        }else{
            return -1;
        }
    }
}
