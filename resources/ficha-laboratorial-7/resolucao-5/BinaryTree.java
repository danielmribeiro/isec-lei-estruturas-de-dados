...
public class BinaryTree<T extends Comparable<? super T>> implements Iterable {
    ...
    private Node<T> Remove(Node<T> subTRoot, T value) {
        ...
        // Remover (cmp == 0)
        // Ṇo tem filhos
        if (subTRoot.getLeft() == sentinel && subTRoot.getRight() == sentinel) {//sem filhos
            return sentinel;
        }

        // Tem, pelo menos, um filho
        if (subTRoot.getLeft() == sentinel) {//so tem 1 filho
            return subTRoot.getRight();
        }
        if (subTRoot.getRight() == sentinel) {//so tem 1 filho
            return subTRoot.getLeft();
        }
        // Tem 2 filhos
        //System.out.println("Tem 2 filhos, o "+root.get()); //Se chegarmos aqui, tem 2 filhos
        Node<T> node = GetMinimumElement(subTRoot.getRight()); //encontrar minimo da subarvore direita para substituir pai
