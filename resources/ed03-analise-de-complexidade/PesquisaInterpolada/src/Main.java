...
public class Main {
    public static void main(String[] args) {
        int[] array={13,21,34,55,69,73,84,101};
        int chave = 73;  //valor de comparação

        System.out.println("Array Original:\t"+imprimeArray(array)+"\n");
        Arrays.sort(array);
        System.out.println("Array Ordenado:\t"+imprimeArray(array)+"\n");
        System.out.println(pesquisaInterpolada(chave,array));
    }
    ...
    private static String pesquisaInterpolada(int chave, int[] array) {
        Arrays.sort(array);
        int min=0;
        int max=array.length-1;
        int iteracoes = 0;
        while(chave >= array[min] && chave<=array[max] && min<=max){
            iteracoes=++iteracoes;
            int tentativa = min + (max-min)*(chave-array[min]) / (array[max]-array[min]);
